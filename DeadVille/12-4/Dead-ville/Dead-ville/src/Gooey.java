...
public class Gooey {
	...
	{		
		...
		mainTextArea.setLineWrap(true);								//allow for word wrapping
		mainTextArea.setWrapStyleWord(true);						//keep whole words together on word wrap
	...
	{			
		mainTextArea.insert(getEventText(), 0); //inserts the event text at the top of the text area
		mainTextArea.setCaretPosition(0);		//keeps the caret at the top so the scroll bar stays at the top
	}
